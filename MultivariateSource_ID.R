...
######## The Claude's vector angle thing
angle <- function(v1, v2) {temp <- sum(v1*v2)/( sqrt(sum(v1^2))*sqrt(sum(v2^2)) )
acos (temp)}


############# One more alternative version of the ang.vec function

ang.vec.alt <- function(vec1, vec2){
	# In this version the sign of the first element of each vector is tested. 
	# If it is negative, the whole vector is multiplied by -1.
	# This should remove simple arbitrary sign changes in the vector
	
  # Check sign of first element of vector, if negative multiply vector by 1
    if(vec1[1] <= 0) vec1 <- vec1*(-1)
    if(vec2[1] <= 0) vec2 <- vec2*(-1)
   
  # Now compute as normal  	
	vec.cor <- (t(vec1) %*% vec2)/(PD(vec1)*PD(vec2))
	vec.angle <- acos(vec.cor)*(180/pi)
	return(c(vector.cor=vec.cor, vec.angle=vec.angle))}	
	
comment(ang.vec.alt) <- c("In this version the sign of the first element of each vector is tested." ,
	                      "If it is negative, the whole vector is multiplied by -1.",
	                      "This should remove simple arbitrary sign changes in the vector")
