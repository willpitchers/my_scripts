...
####f3.2
 ...
 zobs<-Angle(ev1, ev2)
  ...
  if (graph)
  {hist(dist,breaks=50,
   main="Distribution of the cosine of the angle between 2 random vectors", xlab="Z statistic",ylab="# of vect. aléaloire", sub=paste("Actual z-obs =",round(zobs,5),": p<",round((1-abs(0.5-pv)),5)))
   abline(v=zobs)}
  list(z.stat=zobs,p=1-(abs(0.5-pv))*2, angle=acos(zobs))}
####f3.3
